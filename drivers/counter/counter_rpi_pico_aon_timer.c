					    ...
					    const struct counter_alarm_cfg_64 *alarm_cfg)
{
	...
	if (alarm_cfg->ticks > counter_rpi_pico_aon_get_top_value(dev)) {
		return -EINVAL;
	}
	...
	if (target + data->guard_period <= current_time) {
		if ((alarm_cfg->flags & COUNTER_ALARM_CFG_EXPIRE_WHEN_LATE) == 0) {
			data->callback = NULL;
			data->user_data = NULL;
		}
...
#define COUNTER_RPI_PICO_AON_TIMER(inst)                                                           \
	...
	static const struct counter_rpi_pico_aon_timer_config counter_##inst##_config = {          \
		...
		.info = {COND_CODE_1(CONFIG_COUNTER_64BITS_TICKS,                                  \
